...
export default async function EditarCompraPage({
  ...
  // Reconstruimos el precio de lista (sin descuento) a partir del costo final guardado.
  const precioListaUnitario = descuento < 100 ? precioCosto / (1 - descuento / 100) : precioCosto;

  return (
    <div className="mx-auto max-w-2xl space-y-4">
      <h1 className="text-xl font-semibold text-gray-900">Editar compra</h1>
      ...
      <form action={actualizarCompra} className="space-y-4 rounded-xl border border-gray-200 bg-white p-4">
        ...
        <div className="space-y-1">
          ...
          <p className="rounded-md border border-gray-200 bg-gray-50 px-3 py-2 text-sm text-gray-700">
            {compra.producto.sku} · {compra.producto.nombre}
          </p>
        </div>
        ...
        <div className="space-y-1">
          ...
          <select
            name="proveedorId"
            required
            defaultValue={compra.proveedorId}
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          ...
          <div className="space-y-1">
            ...
            <input
              name="numeroPedido"
              defaultValue={compra.numeroPedido ?? ""}
              className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm"
            />
          </div>
          ...
          <div className="space-y-1 sm:col-span-2">
            ...
            <input
              name="numeroFactura"
              defaultValue={compra.numeroFactura ?? ""}
              className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm"
            />
          </div>
        </div>
